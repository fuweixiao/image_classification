...
function M.parse(arg)
   local defaultDir = paths.concat(os.getenv('HOME'), 'fbcunn_imagenet')
    ...
    cmd:option('-data',
               defaultDir .. '/imagenet_raw_images/demo',
    ...
    cmd:option('-epochSize',       1, 'Number of batches per epoch')
    ...
    cmd:option('-retrain',     '/home/fwx/fbcunn_imagenet/imagenet_runs/alexnet12/,ThuApr222:40:372015/model_20.t7', 'provide path to model to retrain with')
    ...
    opt.save = paths.concat(opt.cache,
                            cmd:string('alexnet12', opt,
                                       {retrain=true, optimState=true, cache=false, data=true}))
    -- add date/time
